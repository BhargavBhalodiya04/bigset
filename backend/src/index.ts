...
async function ensureLocalSetupReady(reply: FastifyReply): Promise<boolean> {
  try {
    ...
    await reply.code(428).send({
      error: "Local setup is incomplete. Connect TinyFish and at least one AI provider (OpenRouter or OpenAI) first.",
    });
    return false;
  }
}
...
fastify.post("/local-setup/openai-key", async (req, reply) => {
  if (!env.IS_LOCAL_MODE) {
    return reply.code(404).send({ error: "Not found" });
  }
  ...
  if (!apiKey) {
    return reply.code(400).send({ error: "OpenAI API key is required" });
  }

  try {
    await verifyOpenAIApiKey(apiKey, baseUrl);
    await saveLocalCredential("openai", apiKey, "api_key", baseUrl);
    return await getLocalSetupStatus();
  } catch (err) {
    const message = err instanceof Error ? err.message : "OpenAI verification failed";
    req.log.warn({ err }, "OpenAI local setup verification failed");
    return reply.code(400).send({ error: message });
  }
});

fastify.post("/openai/refresh", { preHandler: requireAuth }, async (req, reply) => {
  const { fetchModelsFromOpenAI, upsertOpenAIModelBatch } = await import("./config/models.js");
  try {
    const models = await fetchModelsFromOpenAI();
    await upsertOpenAIModelBatch(models);
    return { success: true, models };
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to refresh OpenAI models";
    req.log.error(err, "OpenAI refresh failed");
    return reply.code(500).send({ error: message });
  }
});

fastify.get("/openai/models", async (req, reply) => {
  const { convex } = await import("./convex.js");
  const { api } = await import("./convex.js");
  try {
    const models = await convex.query(api.openAIModels.list, {});
    return { models };
  } catch (err) {
    req.log.error(err, "Failed to load cached OpenAI models");
    return reply.code(500).send({ error: "Failed to load OpenAI models" });
  }
});
...
await fastify.register(async (instance) => {
  ...
  instance.post("/settings/models", async (req, reply) => {
    ...
    if (toValidate.length > 0) {
      try {
        const models = await getCachedModels(); // returns combined OR + OAI list
