...
function isPlaceholder(value: string, service: LocalCredentialService): boolean {
  ...
  if (service === "openai" && value === "sk-...") return true;
  return false;
}

function envCredential(service: LocalCredentialService): string | undefined {
  let value: string | undefined;
  if (service === "tinyfish") value = process.env.TINYFISH_API_KEY;
  else if (service === "openrouter") value = env.OPENROUTER_API_KEY;
  else if (service === "openai") value = process.env.OPENAI_API_KEY;
...
async function localCredential(service: LocalCredentialService): Promise<{
  ...
  return {
    ...
    baseUrl: (row as Record<string, unknown>)?.baseUrl as string | undefined,
  };
}
...
export async function getOpenAIApiKey(): Promise<string | undefined> {
  return (await resolveCredential("openai"))?.apiKey;
}

export async function requireOpenAIApiKey(): Promise<string> {
  const apiKey = await getOpenAIApiKey();
  if (!apiKey) {
    throw new Error("OpenAI is not configured. Add your OpenAI API key in setup.");
  }
  return apiKey;
}
...
export async function getOpenAIBaseUrl(): Promise<string | undefined> {
  if (env.IS_LOCAL_MODE) {
    const local = await localCredentialForStatus("openai");
    return local?.baseUrl ?? undefined;
  }
...
export async function getLocalSetupStatus(): Promise<LocalSetupStatus> {
  if (!env.IS_LOCAL_MODE) {
    ...
    const openai = envCredential("openai");
    return {
      mode: "production",
      required: false,
      complete: true,
      services: {
        ...
        openai: {
          configured: !!openai,
          source: openai ? "env" : null,
          connectionMethod: openai ? "api_key" : null,
          verifiedAt: null,
        },
      },
    };
  ...
  const openaiLocal = await localCredentialForStatus("openai");
  ...
  const openai: ServiceSetupStatus = openaiLocal
    ? {
        configured: true,
        source: "local",
        connectionMethod: openaiLocal.connectionMethod,
        verifiedAt: openaiLocal.verifiedAt,
      }
    : {
        configured: false,
        source: null,
  ...
  return {
    mode: "local",
    required: true,
    complete: tinyfish.configured && (openrouter.configured || openai.configured),
    services: { tinyfish, openrouter, openai },
  };
}
...
export async function verifyOpenAIApiKey(apiKey: string, customBaseUrl?: string): Promise<void> {
  const baseUrl = (customBaseUrl || process.env.OPENAI_BASE_URL || "https://api.openai.com/v1").replace(/\/+$/, "");
  ...
  await withFetchTimeout(
    async (signal) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `OpenAI verification failed with HTTP ${response.status}.`,
        );
      }
    },
    `OpenAI verification timed out after ${FETCH_TIMEOUT_MS / 1000} seconds.`,
  );
}
