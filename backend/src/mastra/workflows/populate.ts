...
import { getOpenRouterApiKey, getOpenAIApiKey, getOpenAIBaseUrl } from "../../local-credentials.js";
import { createModelInstance } from "../../model-factory.js";
...
Respond with EXACTLY one word: scraper or search`;
    ...
    try {
      const [openrouterKey, openaiKey, openaiBaseUrl] = await Promise.all([getOpenRouterApiKey(), getOpenAIApiKey(), getOpenAIBaseUrl()]);
      const providerKeys = { openrouter: openrouterKey, openai: openaiKey, openaiBaseUrl };
      const modelSlug =
        inputData.authContext?.modelConfig?.schemaInference ?? DEFAULT_MODEL_IDS.SCHEMA_INFERENCE;
      const result = await generateText({
        model: createModelInstance(modelSlug, providerKeys),
  ...
  execute: async ({ inputData }) => {
    ...
    try {
      ...
      const agent = buildPopulateAgent(
        ...
        inputData.columns,
        { openrouter: openrouterKey, openai: openaiKey, openaiBaseUrl },
