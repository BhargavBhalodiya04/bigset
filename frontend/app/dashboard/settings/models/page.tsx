...
export default function ModelSettingsPage() {
  ...
  const convexORModels = useQuery(api.openRouterModels.list, {});
  const convexOAIModels = useQuery(api.openAIModels.list, {});
  ...
  const isLoading = convexORModels === undefined || convexOAIModels === undefined || isLoadingConfig;
  ...
  // Combine both model lists from Convex reactive queries
  const models: OpenRouterModel[] = [
    ...(convexORModels ?? []).map((m) => ({
  ...
  function openSideSheet(role: ModelRole) {
    if (sheetModels.length === 0) {
      // Load both providers on first open
      Promise.all([getOpenRouterModels(), getOpenAIModels()])
        .then(([orModels, oaiModels]) => setSheetModels([...orModels, ...oaiModels]))
        .catch(() => {
          // we will add toast later
        });
    }
    ...
    <SettingsPageLayout navItems={navItems}>
      <div className="w-full max-w-4xl">
        ...
        <SettingsHeader
          ...
          subtitle="Configure AI models for different tasks. Models are fetched from OpenRouter and OpenAI."
        />
        ...
        <ModelSideSheet
          ...
          onRefresh={async () => {
            ...
            try {
              const token = await getToken();
              if (!token) throw new Error("Not authenticated");
              const [orModels, oaiModels] = await Promise.all([
                refreshOpenRouterModels(token).catch(() => [] as OpenRouterModel[]),
                refreshOpenAIModels(token).catch(() => [] as OpenRouterModel[]),
              ]);
              setSheetModels([...orModels, ...oaiModels]);
            } catch {
              // we will add toast later
            } finally {
              setRefreshing(false);
