...
function groupModelsByProvider(models: OpenRouterModel[]): Record<string, OpenRouterModel[]> {
  ...
  for (const model of models) {
    let provider: string;
    if (model.canonicalSlug.startsWith("openai:")) {
      provider = "openai";
    } else {
      provider = model.canonicalSlug.split("/")[0] || "Other";
    }
...
export function ModelSideSheet({
  ...
  const exactMatch = models.some(
    (m) => m.canonicalSlug === search.trim() || m.modelName === search.trim(),
  );
        ...
        {showCustomOption && (
          <div className="px-4 py-3 border-t border-border shrink-0">
            <button
              ...
              disabled={isSaving}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left border border-dashed border-border hover:border-foreground/30 hover:bg-foreground/2 transition-colors disabled:opacity-50"
            >
              <div className="size-4 rounded-full border-2 border-muted/30 shrink-0" />
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-foreground">Use custom model</p>
                <p className="text-[11px] text-muted font-mono truncate">{search.trim()}</p>
              </div>
            </button>
          </div>
        )}
