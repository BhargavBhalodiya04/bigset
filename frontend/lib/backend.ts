...
export async function saveOpenAIApiKey(
  ...
  const res = await fetch(`${BACKEND_URL}/local-setup/openai-key`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
/**
 * Fetch the cached list of OpenAI models from the backend.
 */
export async function getOpenAIModels(): Promise<OpenRouterModel[]> {
  const res = await fetch(`${BACKEND_URL}/openai/models`, {
    method: "GET",
  });
...
/**
 * Refresh the OpenAI model cache by fetching the latest list from the OpenAI API.
 */
export async function refreshOpenAIModels(token: string): Promise<OpenRouterModel[]> {
  const res = await fetch(`${BACKEND_URL}/openai/refresh`, {
    method: "POST",
    headers: {
